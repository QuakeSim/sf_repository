...
public class DislocService extends AntVisco implements Runnable {  
	...
	public DislocResultsBean runNonBlockingDisloc(String userName, String projectName, Fault[] faults, DislocParamsBean dislocParams, String targetName) throws Exception {

		System.out.println("RunNonBlocking called");
		if(targetName==null)
			targetName=DislocConstants.DISLOC_DEFAULT_TARGET;

		String jobStamp=generateJobStamp();
		String[] args=prefabDisloc(userName, projectName, dislocParams, faults, targetName, jobStamp);
		setArgs(args);
		execute();
		return createDislocResultsBean(userName,projectName,jobStamp);
	}

	public DislocResultsBean runBlockingDisloc(String userName, String projectName, Fault[] faults, DislocParamsBean dislocParams, String targetName) throws Exception {
		if(targetName==null)
			targetName=DislocConstants.DISLOC_DEFAULT_TARGET;
		String jobStamp=generateJobStamp();
		String[] args=prefabDisloc(userName, projectName, dislocParams, faults, targetName, jobStamp);
