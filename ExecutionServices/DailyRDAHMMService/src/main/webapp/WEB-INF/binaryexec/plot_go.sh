...
if [ $# == 3 ]; then
  ...
  sed -e 's/INPUT.xyz/'$tmp1'/g;s/INPUT.Q/'$tmp2'/g;s/INPUT.raw/'$tmp3'/g' plot_go_tiny.plt > $1.plt
  sed -e 's/INPUT.xyz/'$tmp1'/g;s/INPUT.Q/'$tmp2'/g;s/INPUT.raw/'$tmp3'/g' plot_xyz_tiny.plt > $1plot.plt
  sed -e 's/INPUT.xyz/'$tmp1'/g;s/INPUT.Q/'$tmp2'/g;s/INPUT.raw/'$tmp3'/g' plot_func.plt > $1plot_func.plt
  `which gnuplot` < $1.plt
  rm $1.plt
  rm $1plot.plt
