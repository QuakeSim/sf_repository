...
public class SimplexBean extends GenericSopacBean {
	...
	private int ARIA_TOKEN_COUNT_V1= 7;
	 ...
	 //Token count for Sar2Simplex input files.  Each line should have 7 columns.
	 private static final int SAR_TOKEN_COUNT=7;
	...
	protected String createNewProject(String projectName) {
	 ...
	 /**
	  * This is called to update project parameters.  See SimplexProjectParams.jsp for 
	...
	public void toggleUpdateFaults(ActionEvent ev) {
	 ...
	 /**
	  * Add SAR2Simplex input format. This is used for uploading input files
	  * generated by the Sar2simplex.py command line script.
	  */
	 public void toggleAddSARObsvForProject(ActionEvent ev){
		currentEditProjectForm.initEditFormsSelection();
		...
		try {
			 ...
			 StringTokenizer st1, st2;
			 st1 = new StringTokenizer(currentEditProjectForm.getObsvSARTextArea().trim(), "\n");
			 String line;
			 int count=0;
			 ...
			 while (st1.hasMoreTokens()) {
				  ...
				  if(line.indexOf("#")>-1) {
						...
						// Should accept spaces, tabs, commas
						st2 = new StringTokenizer(line.trim(), "\t , "); 
						logger.debug("SAR line token count: "+st2.countTokens());
						Observation tmpObsv = new Observation();
						
						if(st2.countTokens()==SAR_TOKEN_COUNT) {
							 String obsvType=st2.nextToken();
							 
							 String x_coord=st2.nextToken();
							 String y_coord=st2.nextToken();
							 String obsvValue=st2.nextToken();
							 String uncertValue=st2.nextToken();
							 String elevation=st2.nextToken();
							 String azimuth=st2.nextToken();
							 
							 tmpObsv.setObsvType(obsvType);							 
							 tmpObsv.setObsvName(projectName + "_obsv_"+count);
							 tmpObsv.setObsvValue(obsvValue);
							 tmpObsv.setObsvError(uncertValue);
							 tmpObsv.setObsvRefSite("1");
							 tmpObsv.setObsvAzimuth(azimuth);
							 tmpObsv.setObsvElevation(elevation);
							 
							 tmpObsv.setObsvLocationEast(x_coord);
							 tmpObsv.setObsvLocationNorth(y_coord);
