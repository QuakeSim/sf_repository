...
<%
	...
	if (dataSource == null) {
		...
		String[] paras = qs.split("&");
		for (int i=0; i<paras.length; i++) {
			System.out.println(paras[i]);
			String p = paras[i];
			if (p.startsWith("dataSource")) {
				int idx = p.indexOf('=');
				if (idx >=0 ) {
					dataSource = p.substring(idx+1);
...
<!-- inline functions about map api -->	
			...
			<td valign="top" width="600">
				...
				<table class="ooib" id="obody" border="0" cellspacing="0" cellpadding="0" width="767" height="635">
					...
					<tr valign="top" style="display: none">
						...
						<table>
							<tr>
								<td colspan="2"><div id="scnPlotDiv" style="width: 759px; height: 539px"></div></td>
							</tr>
							<tr align="center">
	...
	function printNetworkColors(array) {
		var html = "<table border='1'><tr><td><b>State</b></td><td nowrap><b>Icon Color<b></td></tr>";
		var row;
	...
	function scnWholeAreaBtnClick(btn) {
		...
		scnPlotGraph = new Dygraph(document.getElementById("scnPlotDiv"), scnJsiLink,
									{
										colors:['#007FFF'],
										strokeWidth:0.7,
										pixelsPerXLabel:50,
	...
	/* The state change xml file is formated like:
	<xml>
		<output-pattern>
...
%>
	...
	function printChangedStations() {
		...
		var html = "<table id=\"stateChangeTable\" border='0'> <tr><td><b>Stations with state changes:</b></td></tr> "
						+ "<tr><td>(Select one for details)</td></tr> <tr><td><select id=\"stationSelect\" style=\"width:210px\" onChange=\"sltChange(this)\" selectedIndex=0 >";
		for (var i=0; i<stationArray.length; i++) {
			html += "<option>" + stationArray[i][0] + "</option>";
		}
		html += "</td></tr><tr align='center'><td><b>Last 10 State Changes:</b></td></tr> <tr> </tr> <tr> </tr> <tr> </tr> <tr> </tr> </table>";
		idiv.innerHTML = html;
	}
	
	function sltChange(stationSlt) {
		if (stationSlt.selectedIndex < 0) {
			return;
		}
		var initSetup = (selectedStation == null);
		selectedStation = stationArray[stationSlt.selectedIndex];
		...
		var outputTable = "<table border='0'> <tr><td> <b>Output Values</b> </td></tr>" + "<tr><td><a target=\"_blank\" href=\"" 
							+  preFix + inputPattern.replace(/{!station-id!}/g, stationId) + "\">Input File</a></td></tr>"
		...
		var changeTable = "<table border='1'> <tr> <td><b>Date</b></td> <td><b>Old State</b></td> <td><b>New State</b></td> </tr>";
		...
		var tRows = document.getElementById("stateChangeTable").rows;
		tRows[tRows.length - 3].innerHTML = "<td>" + changeTable + "</td>";
		tRows[tRows.length - 1].innerHTML = "<td>" + outputTable + "</td>";

		if (!initSetup) {
			openWinForStation(stationSlt.selectedIndex);
		}
	}

	function openWinForStation(stationIdx) {
