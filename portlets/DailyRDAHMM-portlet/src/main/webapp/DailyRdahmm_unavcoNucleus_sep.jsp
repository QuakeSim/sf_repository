...
<!-- inline functions about map api -->	
	...
	<table align="left" border="0">
		<tr>
			<td width="936" colspan="2">
				<b><font size="4" face="Verdana">Daily RDAHMM GPS Data Analysis - UNAVCO Nucleus Context Group</font></b><p>
	...
	function scnPlotBtnClick(btn) {
		...
		var url = "http://local.hostname/axis2/services/DailyRdahmmResultService/proxyCallHttpService?serviceUrl="
					...
					+ "dataSource%3DunavcoNucleus%26minLat%3D" + latFrom + "%26maxLat%3D" + latTo + "%26minLong%3D" + longFrom + "%26maxLong%3D" + longTo;
	...
	function getKmlBtnClick(btn) {
		...
		var url = "http://local.hostname/axis2/services/DailyRdahmmResultService/proxyCallHttpService?serviceUrl="
				  ...
				  + "%26resUrl%3Dhttp%3A%2F%2Fxml.access.hostname%2F%2Fdaily_rdahmmexec%2Fdaily%2FunavcoNucleusFill%2Fstation-status-change-unavcoNucleusFill.xml";
...
<%
	...
	String xmlUrl = "http://xml.access.hostname//daily_rdahmmexec/daily/unavcoNucleusFill/station-status-change-unavcoNucleusFill.xml";
	Element eleXml = null;
	Element eleOutput = null;
	try {
		// if the file is old or does not exist, copy it from xmlUrl
		boolean shouldCopy = false;		
		File localFile = new File(config.getServletContext().getRealPath("station-status-change-unavcoNucleusFill.xml"));
		if (localFile.exists()) {		
