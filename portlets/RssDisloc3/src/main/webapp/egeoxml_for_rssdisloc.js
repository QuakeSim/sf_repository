...
function EGeoXml(myvar, map, url, opts) {
	 ...
	 this.displacementkmls = [];
	 this.insarkmls = [];
	 
	 this.dkmlurlobjs = [];
	 this.ikmlurlobjs = [];
	 this.dkmlurlobjs_on_themap;
	 this.ikmlurlobjs_on_themap;
}
		  ...
		  while ((result = pattern.exec(text)) != null) {
				...
				if (result2 != null) {
					 ...
					 if (result3 != null) {
						  ...
						  //GLog.write('Preloading "' + imageUrl +'"');
...
EGeoXml.prototype.createPolygon = function(points,color,width,opacity,fillcolor,fillopacity,pbounds, name, desc) {  
	 ...
	 var clickpolyobjfn = this.clickpolyobjfn;
	 var iwoptions = this.opts.iwoptions || {};
	 var p = new GPolygon(points,color,1,opacity,fillcolor,fillopacity)
	 this.map.addOverlay(p);
...
//This method does the actual document grokking.
EGeoXml.prototype.processing = function(doc) {
					 ...
					 if (!!that.opts.baseicon) {
						  ...
						  if (!!that.opts.noshadow) {
								...
								if (href.indexOf("/red.png")>-1 
									 ...
									 that.styles["#"+styleID].shadow="http://maps.google.com/mapfiles/ms/icons/msmarker.shadow.png";
								}
								else if (href.indexOf("-pushpin.png")>-1) {
									 that.styles["#"+styleID].shadow="http://maps.google.com/mapfiles/ms/icons/pushpin_shadow.png";
								}
								else {
    ...
    for (var i = 0; i < placemarks.length; i++) {
        ...
        var style=GXml.value(placemarks[i].getElementsByTagName("styleUrl")[0]);
        ...
        if (path.length > 1) {
				...
				// [CGL Version] Now we parse alternative also but won't use it. 08/20/2010
				...
				if (linestring.length) {	
					 var coords=GXml.value(placemarks[i].getElementsByTagName("LineString")[0].getElementsByTagName("coordinates")[0]);
          ...
          // Does the user have their own createmarker function?
				...
				if (polygons.length) {
					 ...
					 var coords=GXml.value(placemarks[i].getElementsByTagName("Polygon")[0].getElementsByTagName("coordinates")[0]);
    ...
    for (var i = 0; i < grounds.length; i++) {
        ...
        var url=GXml.value(grounds[i].getElementsByTagName("href")[0]);
    ...
    var displacementkmls = xmlDoc.documentElement.getElementsByTagName("DisplacementKmlURL");    
    var insarkmls = xmlDoc.documentElement.getElementsByTagName("InsarKmlURL");
    ...
    if (true) {
        ...
        if (that.opts.sidebarid) {
				...
				if (this.urls[n].indexOf("/RssDisloc3/") >= 0) {
					 var temp_url_bits = this.urls[n].split("/RssDisloc3/",2);
				...
				var side_bar_list_ordered = [];
				var side_bar_list_ordered_html = [];
				
				for (var i=this.overlaycounter; i<that.side_bar_list.length; i++) {
					 var bits = that.side_bar_list[i].split("$$$",4);
					 // var pname = bits[0].substring(0, bits[0].length-1);
					 var pname = bits[0].split("_n_")[0];
					 ...
					 // bits[3] = graphic
					 var idx = -1;
					 for (var nA = 0 ; nA < side_bar_list_ordered.length ; nA++) {
						  if (side_bar_list_ordered[nA] == pname)
								idx = nA;
					 }
					 
					 if (idx == -1) {
						  side_bar_list_ordered.push(pname);
						  idx = side_bar_list_ordered.length - 1;
						  var html_temp = '<li>'+pname;
						  side_bar_list_ordered_html[idx] = html_temp;
					 }
					 ...
					 side_bar_list_ordered_html[idx] = html_temp;
					 //console.log("Displacement KML:"+this.displacementkmls[i]);
					 //console.log("InSAR KML:"+this.insarkmls[i]);
				}
				
				for (var nA = 0 ; nA < side_bar_list_ordered_html.length ; nA++) {
...
that.side_bar_html += 
'<input type="checkbox" id="li_' + url_bits[0] + '"' + 'style="visibility:hidden" onchange="' + this.myvar + '.togglechboxofAllOverlays(' + '\'' + url_bits[0] + '\',' + this.overlaycounter + ')" checked/>' +
'<input type="checkbox" id="li_' + url_bits[0] + '_1"' + 'style="visibility:hidden" onchange="' + this.myvar + '.togglechboxofAllOverlays(' + '\'' + url_bits[0] + '_1\',' + this.overlaycounter + ')" checked/>' + 
'<input type="checkbox" id="li_' + url_bits[0] + '_2"' + 'style="visibility:hidden" onchange="' + this.myvar + '.togglechboxofAllOverlays(' + '\'' + url_bits[0] + '_2\',' + this.overlaycounter + ')" checked/>';
	...
	// document.getElementById('li_' + url_bits[0]).checked = false;

	if (that.opts.off) {
	  document.getElementById('li_' + url_bits[0]).click();

          
	    document.getElementById('li_' + url_bits[0] + '_1').click();
	    document.getElementById('li_' + url_bits[0] + '_2').click();	    
	    // this.togglechboxofAllOverlays('li_' + url_bits[0] + '_1', this.overlaycounter);	    
	    // this.togglechboxofAllOverlays('li_' + url_bits[0] + '_2', this.overlaycounter);	    
	    // this.togglechboxofAllOverlays('li_' + url_bits[0] + '_3', this.overlaycounter);
...
EGeoXml.prototype.toggleDisplacementKml = function(overlayid, i, url) {

	 console.log("ToggleDisplacementKML called:"+overlayid+" "+i+" "+url);
	 //Remove the previous overlay
	 /*
	 if (this.dkmlurlobjs_on_themap != null){
		  console.log("Removing previous map overlay");
		  this.map.removeOverlay(this.dkmlurlobjs_on_themap);	
	 }
	 */
	 if(!document.forms[this.opts.parentformofsidebarid]){
		  console.log("Parent form of sidebarId not found");
		  return;
	 }
	 
	 var objCheckBoxes = document.forms[this.opts.parentformofsidebarid].elements[overlayid + '_1_overlays'];
	 var countCheckBoxes = objCheckBoxes.length;	
	 console.log("Number of checkboxes: "+countCheckBoxes);
	 
	 if(objCheckBoxes[i].checked==true) {
		  console.log("Adding displacement overlay for "+url);
		  this.dkmlurlobjs[i] = new GGeoXml(url);
		  console.log("Overlay has loaded:"+this.dkmlurlobjs[i].hasLoaded());
		  this.map.addOverlay(this.dkmlurlobjs[i]);
		  this.dkmlurlobjs[i].gotoDefaultViewport(this.map);
	 }
	 else if(objCheckBoxes[i].checked==false) {
		  console.log("Removing overlay:"+i);
		  this.map.removeOverlay(this.dkmlurlobjs[i]);
	 }
	 
/*
  for(var nA=0;nA<objCheckBoxes.length;nA++) {
		  if(objCheckBoxes[nA].checked==true) {
				console.log("Adding displacement overlay for "+url);
				this.dkmlurlobjs[nA] = new GGeoXml(url, function(){
					 while(!(this.dkmlurlobjs[nA]).hasLoaded()) {
						  console.log("Overlay loading");
					 }
				});
				console.log("Overlay has loaded");
				this.map.addOverlay(this.dkmlurlobjs[nA]);
//				this.dkmlurlobjs_on_themap = this.dkmlurlobjs[i];
		  }
		  //The box isn't checked.
		  else {
				//Don't worry if the overlay hasn't been previously added.  Just fail
				//gracefully.
				console.log("Removing overlay:"+nA);
				this.map.removeOverlay(this.dkmlurlobjs[nA]);
		  }
	 }
*/
/*	 
	 for(var nA = 0; nA < objCheckBoxes.length; nA++) {
		  if (nA != i) {
				objCheckBoxes[nA].checked = false;	
		  }
		  else {
				if(objCheckBoxes[nA].checked != false) {
					 console.log("Adding displacement overlay for "+url);
					 this.dkmlurlobjs[i] = new GGeoXml(url, function(){
						  while(!(this.dkmlurlobjs[i]).hasLoaded()) {
								console.log("Overlay loading");
						  }
					 });
					 console.log("Overlay has loaded");
...
EGeoXml.prototype.toggleInsarKml = function(overlayid, i, url) {
	 console.log("ToggleInsarKML called:"+overlayid+" "+i+" "+url);	 

	 if (this.ikmlurlobjs_on_themap != null)
		  this.map.removeOverlay(this.ikmlurlobjs_on_themap);
	 
	 if(!document.forms[this.opts.parentformofsidebarid])
		  return;
	 
	 var objCheckBoxes = document.forms[this.opts.parentformofsidebarid].elements[overlayid + '_2_overlays'];
	 var countCheckBoxes = objCheckBoxes.length;	
	 
	 if(objCheckBoxes[i].checked==true) {
		  console.log("Adding insar overlay for "+url);
		  this.ikmlurlobjs[i] = new GGeoXml(url);
		  console.log("Overlay has loaded:"+this.ikmlurlobjs[i].hasLoaded());
		  this.map.addOverlay(this.ikmlurlobjs[i]);
		  this.ikmlurlobjs[i].gotoDefaultViewport(this.map);
	 }
	 else if(objCheckBoxes[i].checked==false) {
		  console.log("Removing overlay:"+i);
		  this.map.removeOverlay(this.ikmlurlobjs[i]);
	 }
}
